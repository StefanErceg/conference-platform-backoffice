...
export const Countries: FC = () => {
        ...
        if (updated !== null) {
            if (countries?.find(({ id }) => id === updated?.id)) {
                setCountries((countries) =>
                    countries?.map((country) => (country?.id === updated?.id ? updated : country))
                );
    ...
    const deleteCountry = (id: number) => {
        setCountries((countries) => countries.filter((country) => country?.id !== id));
    };
    ...
    return (
        <Loader loaded={loaded}>
            ...
            {modalOpened ? (
                <CountryModal close={closeModal} country={selectedCountry} updateCountries={updateCountries} />
            ) : null}
        </Loader>
    );
};
